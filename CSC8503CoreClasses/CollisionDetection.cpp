...
bool CollisionDetection::RayIntersection(const Ray& r, GameObject& object, RayCollision& collision) {
	...
	case VolumeType::Fluid: hasCollided = RayFluidIntersection(r, worldTransform, (const FluidVolume&)*volume, collision); break;
	}


	return hasCollided;
}
bool CollisionDetection::RayFluidIntersection(const Ray& r, const Transform& worldTransform, const FluidVolume& volume, RayCollision& collision) {
	Vector3 boxPos = worldTransform.GetPosition();
	float boxSize = volume.GetHalfHeight();
	return RayBoxIntersection(r, boxPos, Vector3(boxSize, boxSize, boxSize), collision);
}
...
bool CollisionDetection::AABBIntersection(const AABBVolume& volumeA, const Transform& worldTransformA,
	...
	if (overlap) {
		...
		float distances[6] = {  // distance of:
			(maxB.x - minA.x), // box B to left of box A
			(maxA.x - minB.x), // box B to right of box A
...
bool CollisionDetection::FluidAABBIntersection(const AABBVolume& volumeA, const Transform& worldTransformA,
	const FluidVolume& volumeB, const Transform& worldTransformB, CollisionInfo& collisionInfo){
	Vector3 boxAPos = worldTransformA.GetPosition();
	Vector3 boxBPos = worldTransformB.GetPosition();

	Vector3 boxASize = volumeA.GetHalfDimensions();
	float boxBHeight = volumeB.GetHalfHeight();
	Vector3 boxBSize(boxBHeight, boxBHeight, boxBHeight);
